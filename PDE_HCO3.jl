function PDE_HCO3!(dX, C, CO3, H2CO3, Ca, Temperature, params, t)
    ...
    for i in 0:Ny
        for j in 0:Nz
            Y[pos2idx(i,j)] = pH_o + C[pos2idx(i,j)]*hco3_to_M - HCO3_o*hco3_to_M + 2*CO3[pos2idx(i,j)]*co3_to_M - 2*CO3_o*co3_to_M - 2*Ca[pos2idx(i,j)]*ca_to_M + 2*Ca_o*ca_to_M
        end
    end
    ...
    for i=1:Ny
        ...
        dHCO3[pos2idx(i,Nz)] =  (  (k3*H2CO3[pos2idx(i,Nz)]*h2co3_to_M - kneg3*Y[pos2idx(i,Nz)]*C[pos2idx(i,Nz)]*hco3_to_M - k4*C[pos2idx(i,Nz)]*hco3_to_M + kneg4*Y[pos2idx(i,Nz)]*CO3[pos2idx(i,Nz)]*co3_to_M)*(1/hco3_to_M) #reaction kinetics of H2CO3
    ...
    for i=1:Ny
        for j=0:Nz-1
            dHCO3[pos2idx(i,j)] =  (  (k3*H2CO3[pos2idx(i,j)]*h2co3_to_M - kneg3*Y[pos2idx(i,j)]*C[pos2idx(i,j)]*hco3_to_M - k4*C[pos2idx(i,j)]*hco3_to_M + kneg4*Y[pos2idx(i,j)]*CO3[pos2idx(i,j)]*co3_to_M)*(1/hco3_to_M) #reaction kinetics of H2CO3
    ...
    @views dX[1+4*Nelements:5*Nelements] .= dHCO3        # order matters! The @views operator takes a slice out of an array without making a copy.
    nothing
end