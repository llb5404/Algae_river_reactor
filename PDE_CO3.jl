function PDE_CO3!(dX, C, HCO3, Ca, Temperature, params, t)
    ...
    for i in 0:Ny
        for j in 0:Nz
            Y[pos2idx(i,j)] = pH_o + HCO3[pos2idx(i,j)]*hco3_to_M - HCO3_o*hco3_to_M + 2*C[pos2idx(i,j)]*co3_to_M - 2*CO3_o*co3_to_M - 2*Ca[pos2idx(i,j)]*ca_to_M + 2*Ca_o*ca_to_M
        end
    end
    ...
    for i in 0:Ny
        for j in 0:Nz
            Z[pos2idx(i,j)] = CaCO3_o*caco3_to_M - Ca[pos2idx(i,j)]*ca_to_M + Ca_o*ca_to_M
        end
    end
    ...
    for i=1:Ny
        ...
        dCO3[pos2idx(i,Nz)] =  (  (k4*HCO3[pos2idx(i,Nz)]*hco3_to_M - kneg4*Y[pos2idx(i,Nz)]*C[pos2idx(i,Nz)]*co3_to_M - k5*C[pos2idx(i,Nz)]*Ca[pos2idx(i,Nz)]*co3_to_M*ca_to_M + kneg5*min(sub,Z[pos2idx(i,Nz)]))*(1/co3_to_M) #reaction kinetics of H2CO3
    ...
    for i=1:Ny
        for j=0:Nz-1
            dCO3[pos2idx(i,j)] =  (  (k4*HCO3[pos2idx(i,j)]*hco3_to_M - kneg4*Y[pos2idx(i,j)]*C[pos2idx(i,j)]*co3_to_M - k5*C[pos2idx(i,j)]*Ca[pos2idx(i,j)]*co3_to_M*ca_to_M + kneg5*min(sub,Z[pos2idx(i,j)]))*(1/co3_to_M) #reaction kinetics of H2CO3
    ...
    @views dX[1+5*Nelements:6*Nelements] .= dCO3        # order matters! The @views operator takes a slice out of an array without making a copy.
    nothing
end