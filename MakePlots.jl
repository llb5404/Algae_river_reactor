    ...
    for i in 1:TL
        for j in 0:Ny
            for k in 0:Nz
                    ...
                    Prod_vec[i,pos2idx1(k)] = ((Mout[i,pos2idx(Ny-1,k)]*Q(Tout[i,pos2idx(Ny-1,0)],WNDSPDout[i],RHout[i],Pa_out[i],Ny-1) - Cinit*params.volumetric_flow_rate_o)* 24.0* 1000)/(W*L)
