...
function PDE_H2CO3!(dX, C, CO2, HCO3, CO3, Ca, Temperature, params, t)
    ...
    for i in 0:Ny
        for j in 0:Nz
            Y[pos2idx(i,j)] = pH_o + HCO3[pos2idx(i,j)]*hco3_to_M - HCO3_o*hco3_to_M + 2*CO3[pos2idx(i,j)]*co3_to_M - 2*CO3_o*co3_to_M - 2*Ca[pos2idx(i,j)]*ca_to_M + 2*Ca_o*ca_to_M
        end
    end
    ...
    for i=1:Ny
        ...
        dH2CO3[pos2idx(i,Nz)] =  (  (k2(Temperature[pos2idx(i,Nz)])*CO2[pos2idx(i,Nz)]*co2_to_M - kneg2(Temperature[pos2idx(i,Nz)])*C[pos2idx(i,Nz)]*h2co3_to_M - k3*C[pos2idx(i,Nz)]*h2co3_to_M + kneg3*Y[pos2idx(i,Nz)]*HCO3[pos2idx(i,Nz)]*hco3_to_M)*(1/h2co3_to_M) #reaction kinetics of H2CO3
    ...
    for i=1:Ny
        for j=0:Nz-1
            dH2CO3[pos2idx(i,j)]=  ( (k2(Temperature[pos2idx(i,j)])*CO2[pos2idx(i,j)]*co2_to_M - kneg2(Temperature[pos2idx(i,j)])*C[pos2idx(i,j)]*h2co3_to_M - k3*C[pos2idx(i,j)]*h2co3_to_M + kneg3*Y[pos2idx(i,j)]*HCO3[pos2idx(i,j)]*hco3_to_M)*(1/h2co3_to_M) #reaction kinetics of H2CO3
    ...
    @views dX[1+3*Nelements:4*Nelements] .= dH2CO3        # order matters! The @views operator takes a slice out of an array without making a copy.
    nothing
end